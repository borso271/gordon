...
function isMarketOpenNow(asset_type, exchange_mic = null): boolean {
    if (asset_type == "crypto") {
      ...
      const marketHolidays = ["1-1", "1-20", "2-17", "4-18", "5-26", "6-19", "7-4", "9-1", "11-27", "12-25"];
      const earlyClosureDays = ["7-3", "11-28", "12-24"];
      const dateKey = `${month}-${day}`;
  
      if (dayOfWeek === 0 || dayOfWeek === 6 || marketHolidays.includes(dateKey)) {
        return false;
      }
